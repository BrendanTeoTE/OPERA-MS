...
#############################################################
my ($inter_dir, $ref_map_folder, $mapping_folder, $sigma_folder, $contig_seq_file, $nb_process, $opera_ms_db, $dist_est, $opera_ms_dir, $mash_exe_dir, $sylph_exe_dir, $skani_exe_dir, $mummer_dir, $perl_exe_dir, $kraken_exe_dir) = @ARGV;
...
#    run_mash_on_clusters(\%cluster_info_, \%contig_info_, "$opera_ms_db/genomes.msh");
    if ($dist_est eq "mash") {
        ...
        run_skani_on_clusters(\%cluster_info_, \%contig_info_, "$opera_ms_db/skani.database", "$opera_ms_db/database.syldb");
    }
...
sub run_skani_on_clusters{
    my ($cluster_info, $contig_info, $skani_ref, $sylph_ref) = @_;
    ...
    my $partial_sketch_dir = "$skani_dir/PARTIAL_SKETCH/";
    run_exe("mkdir -p $partial_sketch_dir");
    ...
    #get list of samples and genomes
    #system("perl -F'\t' -lane 'next if \$\. == 1; print \$F[0]' $skani_dir/sylph_query.out | sort -u > $skani_dir/genome_files.txt");
    ...
    #get references needed for skani and put them in subsetted directory
    #run_exe("mkdir -p $skani_dir/skani_small_db");
    ...
    #while (my $line = <$fh>) {
        ...
        #my ($basename) = $line =~ /([^\/]+)$/;
        #my $source_path = "$skani_ref/$basename.sketch";
        #my $dest_path   = "$skani_dir/skani_small_db/$basename.sketch";

        # Create symlink
        #run_exe("ln -s $source_path $dest_path");
    #}
