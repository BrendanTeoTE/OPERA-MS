...
#############################################################
my ($inter_dir, $ref_map_folder, $mapping_folder, $sigma_folder, $contig_seq_file, $nb_process, $opera_ms_db, $dist_est, $sr1, $sr2, $opera_ms_dir, $mash_exe_dir, $sylph_exe_dir, $skani_exe_dir, $mummer_dir, $perl_exe_dir, $kraken_exe_dir) = @ARGV;
...
my $skani_dir = "$ref_map_folder/SKANI/";
my $sylph_dir = "$ref_map_folder/SYLPH/";
...
#    run_mash_on_clusters(\%cluster_info_, \%contig_info_, "$opera_ms_db/genomes.msh");
    ...
    elsif ($dist_est eq "skani") {
        run_skani_on_clusters(\%cluster_info_, \%contig_info_, "$opera_ms_db/skani.database", "$opera_ms_db/database.syldb", $sr1, $sr2);
    }
...
if ($dist_est eq "mash") {
    ...
    generate_clusters_species_skani(\%cluster_species_set_, \%species_to_ignore_, \%cluster_info_);;
}
...
sub generate_clusters_species_skani {
    my ($cluster_species_set, $species_to_ignore, $cluster_info) = @_;
    my $top_prediction = 5;
    my $nb_prediction;
    ...
    print STDERR " *** Read SKANI files\n";
    my @line;my $species;
    open(FILE, "sort -k3,3gr $skani_dir/skani_dist.out.tsv |");
    while(<FILE>){
	@line = split(/\t/, $_);
	@tmp = split(/\//, $line[1]);
	$cluster_name = $tmp[@tmp-1];
...
sub run_skani_on_clusters{
    my ($cluster_info, $contig_info, $skani_ref, $sylph_ref, $sr1, $sr2) = @_;
    run_exe("rm -r $skani_dir");
    run_exe("mkdir -p $skani_dir");
    my $out;
 
    my $inter_fa_dir = "$skani_dir/INTER_FILES/";
    run_exe("mkdir -p $inter_fa_dir");
    my $sylph_sketch_dir = "$skani_dir/SYLPH_SKETCH/";
    run_exe("mkdir -p $sylph_sketch_dir");
    ...
    #Generation of the cluster fasta fil
    my %cluster_length = ();
    my @tmp;my $cluster_name;my $contig_cluster;
    my $cmd = "";
    #my $max_genome_for_sketch = 500;

    #accumulating sequences into the appropriate cluster buckets using contig info, write out FASTA for each cluster
    ...
    my $count = 0;
    opendir(my $dh, $inter_fa_dir) or die "Cannot open directory $inter_fa_dir: $!";
    while (my $file = readdir($dh)) {
        next if $file =~ /^\./;  # skip . and ..
        my $path = File::Spec->catfile($inter_fa_dir, $file);
        $count++ if -f $path;  # only count regular files
    }
    closedir($dh);

    open(my $log_fh, '>', "$skani_dir/skani_sylph_log.txt") or die "Could not open log file: $!";
    print $log_fh "[Number of files in $inter_fa_dir: $count]\n";

    print $log_fh "[" . localtime() . "] start sylph\n";
    # Run Sylph on raw reads
    run_exe("$opera_ms_dir/bin/run_sylph_raw.sh $inter_fa_dir $sylph_sketch_dir $nb_process $skani_dir $sylph_ref $sr1 $sr2");
    print $log_fh "[" . localtime() . "] end sylph\n";

    my $skani_sketch_dir = "$skani_dir/SKANI_SKETCH/";

    # Extract list of samples and genomes
    #run_exe("perl -F'\\t' -lane 'next if \$\. == 1; print \$F[0]' $skani_dir/sylph_query.out | sort -u > $skani_dir/sample_files.txt");
    run_exe("perl -F'\\t' -lane 'next if \$\. == 1; print \$F[1]' $skani_dir/sylph_query.out | sort -u > $skani_dir/genome_files.txt");

    # Create db_files.txt from genome list
    open(my $fh, '<', "$skani_dir/genome_files.txt") or die "Could not open file: $!";
    open(my $out_fh, '>', "$skani_dir/db_files.txt") or die "Could not write to output file: $!";
    ...
    while (my $line = <$fh>) {
        ...
        my ($basename) = $line =~ /([^\/]+)$/;
        my $full_path = "$skani_ref/$basename.sketch";
        print $out_fh "$full_path\n";
    }
    close($fh);
    close($out_fh);

    # Run Skani
    print $log_fh "[" . localtime() . "] start skani\n";
    run_exe("$opera_ms_dir/bin/run_skani.sh $skani_dir $skani_sketch_dir $nb_process $inter_fa_dir");
    print $log_fh "[" . localtime() . "] end skani\n";
