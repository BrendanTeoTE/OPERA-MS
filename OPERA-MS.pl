...
#To setup and check if the depndency are functioning
...
print "INTER_DIR = ", $opera_ms_option{"INTER_DIR"}, "\n";
...
my $step = $opera_ms_option{"STEP_NUM"} || 0;
...
if ($step == 2 || $step == 0) {
    ...
    my $inter_dir = $opera_ms_option{"INTER_DIR"};
	print "INT_DIR = ($inter_dir]\n";
	my $opera_ms_dir = $opera_ms_option{"OPERA_MS_DIR"};
    my $cov_dir = $opera_ms_option{"INTER_DIR"} . "/coverage_estimation";
	print "COV_DIR = $cov_dir\n";
    $opera_ms_option{"COV_DIR"} = $cov_dir;
	print "COV_DIR = ", $opera_ms_option{"COV_DIR"}, "\n";
...
sub setup_directory_path{
	...
	print "MAIN_DIR = ", $opera_ms_option->{"MAIN_DIR"}, "\n";
...
sub check_dependency{
	...
	#sylph
    check_software($utils_dir, "sylph", "sylph --version", $opera_ms_dependency, $stage);

	#skani
    check_software($utils_dir, "skani", "skani --version", $opera_ms_dependency, $stage);
...
sub check_database{
	...
	my $dist_est = $opera_ms_option->{"DIST_EST"};

	if ($dist_est eq "mash") {
		return (-e "$opera_ms_db_dir/genomes.msh");
	}
	elsif ($dist_est eq "skani") {
		return (-e "$opera_ms_db_dir/skani.database");
		return (-e "$opera_ms_db_dir/database.syldb");
	}
	else {
		die "Unknown DIST_EST method: $dist_est\n";
	}
}
...
sub assembly_graph_creation{
	...
	if(! check_completed($cov_dir."/".$opera_ms_option{"CONTIGS_COV_FILE"}, "Short-read mapping and coverage estimation", 3)){
	    ...
	    my $acf = $opera_ms_option->{"ASSEMBLED_CONTIG_FILE"};
	    print "debug_debug $acf done \n";
...
sub reference_clustering{
	...
	if(! check_completed("$ref_clustering_dir/cluster_species.dat", "Reference based clustering", 5, !$opera_ms_option->{"REF_CLUSTERING"})){
	    ...
	    run_exe($opera_ms_dependency->{"perl"}."perl ${opera_ms_dir}bin/sequence_similarity_clustering.pl " . 
			...
			$opera_ms_option->{"DIST_EST"} . " " .
			$opera_ms_option->{"ILLUMINA_READ_1"} . " " .
			$opera_ms_option->{"ILLUMINA_READ_2"} . " " .
		    $opera_ms_dir . " " . 
		    $opera_ms_dependency->{"mash"} . " " .
	        $opera_ms_dependency->{"sylph"} . " " .
			$opera_ms_dependency->{"skani"} . " " .
...
sub set_default_value{
	...
	$opera_ms_option{"DIST_EST"} = "skani";
...
sub read_argument{
    ...
    # step option for snakemake control
    "step=i" => \$opera_ms_option->{"STEP_NUM"},
	"dist-est=s" => \$opera_ms_option{"DIST_EST"},
	#
